...
class MovingAverages:
    ...
    @property
    def self_defined_ma(self):
        return self.closes.rolling(window=self.window).mean()
...
class Sefi:
    ...
    def get_sefi(self) -> list:
        return np.vectorize(self.evaluate)(self.df['Close'], self.df[self.ma_col])

    @staticmethod
    def evaluate(close: float, ma_n: float) -> Union[float, int]:
        return 1 if close > ma_n else 0


# MARKETS ONLY
class ADR:
    def __init__(self, market_data: SP500Database, dates):
